...
/* Task-card right-click → in-app menu (Edit / Duplicate / Move Group /
   Select Multiple / Delete). When the visual editor (css panel) is
   active, its own context menu takes over. Right-click on non-card
   elements falls through to the browser's native menu. */
...
/* Alt+right-click anywhere → instant copy CSS selector to clipboard. No
   menu shown. Multi-element accumulation: while Alt is held, each Alt+
   right-click appends another selector (newline-separated) to a buffer.
   Buffer is flushed to clipboard when Alt is released or on first
   non-Alt action. Works regardless of whether the css panel is on. */
(function altCopySelector(){
  let altBuffer = [];
  let altFlushTimer = null;
  ...
  async function flushBuffer(){
    if(altBuffer.length === 0) return;
    const txt = altBuffer.join('\n');
    ...
    if(typeof toast === 'function'){
      toast('Copied ' + altBuffer.length + ' selector' +
            (altBuffer.length === 1 ? '' : 's'), false);
    }
    altBuffer = [];
  }
  ...
  document.addEventListener('contextmenu', function(e){
    ...
    clearTimeout(altFlushTimer);
    altFlushTimer = setTimeout(flushBuffer, 250);
  }, true);

  // Releasing Alt key flushes immediately
